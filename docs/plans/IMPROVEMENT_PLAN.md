...
| **Missing finalizer** | `main.go:168-170` | No cleanup when VmwareCloudFoundationMigration is deleted |
...
**File:** `cmd/vmware-cloud-foundation-migration/main.go`
...
2. **Add TLS config to VmwareCloudFoundationMigrationSpec:**
```go
...
```go
func (m *RestoreManager) RestoreAllBackups(ctx context.Context, migration *VmwareCloudFoundationMigration) error {
    var errs []error
    // ... restore loop
...
**Problem:** No cleanup when VmwareCloudFoundationMigration is deleted (vSphere tags/folders remain orphaned).
...
```go
func (c *MigrationController) syncMigration(ctx context.Context, migration *VmwareCloudFoundationMigration) error {
    // Handle deletion
...
func (m *BackupManager) AddBackupToMigration(migration *VmwareCloudFoundationMigration, backup *BackupManifest) error {
...
```go
import "github.com/openshift/vmware-cloud-foundation-migration/pkg/util"
